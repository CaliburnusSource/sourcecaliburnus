$(function() {
     $("#loading-mask").ajaxStart(function(){
       $(this).show();
     });

     $("#loading-mask").ajaxStop(function(){
       $(this).hide();
     });
     ...
     $("form:not(.ajaxible)").live('submit', function(){
         $("#loading-mask").show();
     ...
     $('input[type="password"]').parents('form:first').attr('autocomplete', 'off');

     $("form.jform_select select").change(function() {
         $.post(
            $(this).parent('form').attr('action') + '/format/json',
            $(this).serialize(),
            function(json) {
                if(json.success) {
                    location.reload();
     ...
     */
    if($('#mb').length) {
        $("a.jdax, a.visibility-icon").live('click', function() {
            var opts = {
                autoResize:true,
                overlayClose:false,
                minWidth: 500,
            ...
            $('#mb').load($(this).attr('href') + '?format=html', function(responseText){
                $(this).modal(opts);
            });
            return false;
        });
    ...
    $('input.generate').live('click', function(){
        ...
        $(this).parents('form').find('input[type="password"]').val(passwd);
        $(this).parents('form').find('input.pass_show').val(passwd);
        $(this).parents('form').find('.generated_pass').show().find('input').val(passwd);
    ...
    $('input.button_link').live('click', function() {
        window.location.href= $(this).attr('name');
        return false;
    });
    
    $('input.button_link_confirm').live('click', function() {
    ...
    if($('textarea.jeditor').length > 0) {

        $('textarea.jeditor').each(function() {
    ...
    //
                // Theme options
                theme_advanced_buttons1 : "bold,italic,underline,justifyleft,justifycenter,justifyright,|,bullist,numlist,removeformat,code,forecolor,fullscreen,formatselect",
    ...
    //			content_css : bb_base_url + "images/style_boxbilling0.css"
...
function keepAlive() {
    $.ajax({
      url: '/index/keep-alive/format/json',
      dataType: 'json',
      global: false
    });
}

jQuery.fordnoxMsg = function(msg, type) {
    if(type == undefined) {
        type = 'success';
    }
